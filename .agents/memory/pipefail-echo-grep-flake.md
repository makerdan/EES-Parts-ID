...
**How to apply:** In bash test suites with pipefail, never pipe a variable into `grep -q`; for literal needles prefer pure-bash `[[ "$haystack" == *"$needle"* ]]` (immune to SIGPIPE and to regex metacharacter surprises); otherwise grep the source file directly (`grep -q pattern "$file"`). Beware: the flake reproduces ONLY under real CPU load (concurrent background jobs) — isolated repro loops pass 30/30, so absence of repro in isolation does not clear the assertion helper. Also: killed test runs leave orphaned post-merge.sh/pnpm children that skew later runs — clean strays before diagnosing "flaky" suites.
