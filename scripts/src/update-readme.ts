...
function readArtifactToml(tomlPath: string): Partial<ArtifactInfo> {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    } else if ((section === "services.production" || section === "services.production.run") && key === "run") {
...
function renderScriptsSection(): string {
  ...
  for (const p of artScripts) {
    const names = Object.keys(p.scripts).sort().map(s => `\`${s}\``).join(", ") || "—";
...
function renderFeaturesSection(): string {
  if (!existsSync(FEATURES_PATH)) return "_(no features manifest found at `scripts/README-FEATURES.json`)_";
