...
async function cleanupRows() {
  await db
    ...
    .where(and(eq(inventoryTable.vendor, "BRIDGEPORT"), inArray(inventoryTable.catalog, [...FIXTURE_CATALOGS])));
}
...
describeIfFixture("POST /api/admin/catalog-pdf/preview + /apply (Bridgeport)", () => {
  ...
  it("multipart preview parses, classifies, and apply enriches the right rows", async () => {
    ...
    const colorSiblings = report.rows.filter(r =>
      /^231-S(BLU|R|Y|W|G|O)$/.test(r.catalogNumber),
    );
