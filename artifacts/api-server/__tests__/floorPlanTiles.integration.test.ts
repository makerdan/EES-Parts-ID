 ...
 * The real database is used
;
 a test floor-plan-meta row is inserted in
...
const mockReadFloorPlanSvg = jest.fn(() =>
  Promise.resolve(Buffer.from(mockFloorPlanSvg, "utf8")),
)
;
...
jest.mock("../src/lib/objectStorage", () => (
{
  ...
  readFloorPlanSvg: mockReadFloorPlanSvg,
...
;
  ...
  mockSharpFn.mockImplementation(() => chain)
;
...
;
  ...
  describe("400 for malformed coordinate segments", () => 
{
...
{
 path: "/api/floor-plan/tiles/1abc/0/0", label: "partially numeric z" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/1abc/0", label: "partially numeric x" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/1abc", label: "partially numeric y" 
}
,
      
{
 path: "/api/floor-plan/tiles/0.5/0/0", label: "decimal z" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0.5/0", label: "decimal x" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/0.5", label: "decimal y" 
}
,
      
{
 path: "/api/floor-plan/tiles/+0/0/0", label: "signed z" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/-0/0", label: "signed x" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/-0", label: "signed y" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/0.PNG", label: "uppercase extension" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/0.png.png", label: "repeated extension" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/0foo.png", label: "malformed extension" 
}
,
      
{
 path: "/api/floor-plan/tiles/0/0/%200", label: "encoded whitespace" 
}
,
      
{
 path: "/api/floor-plan/tiles/0//0", label: "empty x" 
}
,
      
{

        path: "/api/floor-plan/tiles/999999999999999999999999/0/0",
        label: "overflow-like z",
...
;
    ...
    it("returns 400 for an empty y coordinate", async () => 
{

      const res = await supertest(app)
        .get("/api/floor-plan/tiles/0/0/")
        .expect(400)
;
...
;
  ...
  it("does not let malformed coordinates reuse the valid tile cache or ETag", async () => 
{
...
;
    ...
    const validEtag = valid.headers["etag"] as string
;

    const readsAfterValidRequest = mockReadFloorPlanSvg.mock.calls.length
;
...
;
    ...
    const malformed = await supertest(app)
      .get("/api/floor-plan/tiles/0/0/0foo")
      .expect(400)
;


    expect(malformed.headers["etag"]).not.toBe(validEtag)
;

    expect(mockReadFloorPlanSvg.mock.calls.length).toBe(readsAfterValidRequest)
;
