...
import { db, categoryNodeTable, inventoryCategoryTable, inventoryTable } from '@workspace/db';
...
async function cleanupFixtures(): Promise<void> {
  ...
  if (fixtures.length > 0) {
    ...
    await db.delete(inventoryCategoryTable).where(inArray(inventoryCategoryTable.inventoryId, ids));
    await db
...
describe('POST /api/categories/reclassify — auth', () => {
  it('returns 401 without a valid admin token', async () => {
    await supertest(app).post('/api/categories/reclassify').send({ useAi: false }).expect(401);
  });
});
