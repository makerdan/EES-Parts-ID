...
async function cleanup() {
  ...
  await db
    ...
    .where(sql`${inventoryBarcodeTable.barcode} ILIKE ${"%" + BC_PREFIX + "%"} OR ${inventoryBarcodeTable.barcode} IN (${UPC_BREAKER}, ${UPC_RECEPTACLE})`);
  await db
    .delete(inventoryTable)
    .where(sql`${inventoryTable.catalog} LIKE ${BC_PREFIX + "%"}`);
}
...
beforeAll(async () => {
  ...
  const rows = await seedFixtures([
    { vendor: "EATON", catalog: CAT_BREAKER, description: "1P 20A breaker", binLocations: ["A-01"] },
    { vendor: "HUBBELL", catalog: CAT_RECEPTACLE, description: "20A duplex receptacle", binLocations: ["B-02"] },
