...
function makeItem(overrides: Partial<InventoryItem> & { id: number; binLocations: string[] }): InventoryItem {
  return {
    id: overrides.id,
...
describe("buildAisleHierarchy", () => {
  ...
  it("counts a part once at the aisle level even when it has multiple bins in that aisle", () => {
    const items = [
      makeItem({ id: 1, binLocations: ["17-06-907", "17-08-101"] }),
    ];
    const tree = buildAisleHierarchy(items);
  ...
  it("does NOT add a part to Unsorted if it has at least one conforming bin", () => {
    const items = [
      makeItem({ id: 1, binLocations: ["LEGACY-A1", "17-06-907"] }),
    ];
    const tree = buildAisleHierarchy(items);
