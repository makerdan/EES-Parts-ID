...
function renderBar(initial: RefinementState = {}) {
  ...
  return {
    onChange,
    setRefinement,
    getInput: () => screen.getByLabelText("Add keywords to refine the current results") as HTMLInputElement,
...
describe("ResultRefinementBar — Add keywords input", () => {
  ...
  it("debounces typed input into a single onChange with the trimmed value", () => {
    ...
    act(() => { fireEvent.change(getInput(), { target: { value: "b" } }); });
    act(() => { fireEvent.change(getInput(), { target: { value: "bl" } }); });
    act(() => { fireEvent.change(getInput(), { target: { value: " blue " } }); });
