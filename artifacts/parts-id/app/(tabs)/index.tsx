...
async function saveQueryCache(cache: QueryCache): Promise<void> {
  try {
    await AsyncStorage.setItem(QUERY_CACHE_KEY, JSON.stringify(cache));
  } catch {}
}
...
export default function SearchScreen() {
  ...
  const [itemOverrides, setItemOverrides] = useState<Map<number, Partial<InventoryItem>>>(
    () => new Map()
  );
  ...
  const [confThresholdInput, setConfThresholdInput] = useState(
    String(DEFAULT_SETTINGS.defaultConfidenceThreshold)
  );
  ...
  useEffect(() => {
    AsyncStorage.getItem(BROWSE_MODE_KEY)
      .then((raw) => {
        ...
        try {
          ...
          if (raw) {
            ...
            const filteredCached = cached.filter((it) => {
              const text =
                `${it.vendor} ${it.catalog} ${it.description} ${(it.aiKeywords ?? []).join(' ')}`.toLowerCase();
              for (const dim of CHIP_DIMS) {
                const v = f[dim.key];
  ...
  const handleBrowseNodeChange = useCallback(
  ...
  const syncAllInventory = useCallback(
  ...
  useEffect(() => {
    ...
    AsyncStorage.multiGet([FUSE_CACHE_KEY, INVENTORY_VERSION_KEY])
      .then(async ([[, rawItems], [, storedVersion]]) => {
        ...
        const cacheStale =
          serverVersion !== null && (storedVersion === null || serverVersion > storedVersion);
  ...
  const runFuseSearch = useCallback((kw: string): SearchResult[] => {
    ...
    return fuseRef.current.search(kw.trim(), { limit: 30 }).map((r) => ({
      item: r.item,
  ...
  const searchMutation = useSearchInventory({
    mutation: {
      onSuccess: (data) => {
        ...
        setDimensionCounts(
          data.dimensionCounts as Record<string, Record<string, number>> | undefined
        );
        ...
        searchEventIdRef.current =
          (data as unknown as { _telemetry?: { searchEventId?: number | null } })._telemetry
            ?.searchEventId ?? null;
        ...
        fetch(`${API_BASE}/inventory/version`)
          .then((r) =>
            r.ok ? (r.json() as Promise<{ updatedAt: string | null }>) : Promise.reject()
          )
          .then(async (v) => {
            if (!v.updatedAt) {
  ...
  const settingsModalMaxHeight = Math.max(320, windowHeight - insets.top - insets.bottom - 64);
  useEffect(() => {
  ...
  const onRepeatTapReset = useCallback(
  ...
  const rawResults: SearchResult[] =
    mode === 'browse' ? (browseResults ?? []) : (offlineResults ?? displayedResults);
  ...
  const hasResults =
    mode === 'browse' ? browseResults !== null : searchHasSucceeded || offlineResults !== null;
  ...
  const refinementActive = Object.values(refinement).some(
        ...
        <View style={[styles.header, { borderBottomColor: colors.border }]}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
              ...
              {syncProgress ? (
                <Animated.View
                  style={[
                    styles.statusBadge,
                    { backgroundColor: colors.muted, opacity: syncPulse },
                  ]}
                >
                  <ActivityIndicator
                    size={10}
                ...
                <Animated.View
                  style={[
                    styles.statusBadge,
                    { backgroundColor: colors.warning + '22', opacity: syncPulse },
                  ]}
                >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            >
              ...
              <Text style={[styles.logoutBtnLabel, { color: colors.mutedForeground }]}>
                Settings
              </Text>
            </Pressable>
          </View>
          ...
          */}
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                />
                <Text
                  style={[styles.settingsModalTitle, { marginBottom: 0, color: colors.foreground }]}
                >
                  Settings
                </Text>
              </View>
              ...
              >
                ...
                <View style={[styles.settingsRow, { borderColor: colors.border }]}>
                  <View style={{ flex: 1 }}>
                    ...
                    {cacheClearedMsg ? (
                      <Text style={[styles.settingsRowSuccess, { color: colors.success }]}>
                        {cacheClearedMsg}
                      </Text>
                    ) : null}
                  </View>
                  ...
                  >
                    <Text style={[styles.clearCacheBtnText, { color: colors.foreground }]}>
                      Clear
                    </Text>
                  </Pressable>
                </View>
                    ...
                    {(['small', 'normal', 'large'] as TextSize[]).map((sz) => (
                      <Pressable
                        ...
                        style={[
                          ...
                          {
                            backgroundColor:
                              settings.textSize === sz ? colors.primary : colors.muted,
                      ...
                      <Pressable
                        ...
                        style={[
                          ...
                          styles.textSizeBtn,
                          {
                            ...
                            backgroundColor:
                              settings.themeMode === mode ? colors.primary : colors.muted,
                            borderColor:
                              settings.themeMode === mode ? colors.primary : colors.border,
                          },
                        ]}
                      >
                        <Text
                ...
                <View
                  style={[
                    styles.settingsRow,
                    { borderColor: colors.border, flexDirection: 'column', gap: 2 },
                  ]}
                >
                  <Text style={[styles.settingsRowLabel, { color: colors.foreground }]}>
                  ...
                  <View style={styles.confCustomRow}>
                    <Text style={[styles.confCustomLabel, { color: colors.mutedForeground }]}>
                      Custom
                    </Text>
                    <TextInput
              ...
              <View style={[styles.settingsModalFooter, { borderTopColor: colors.border }]}>
                ...
                >
                  <Text
                    style={[styles.logoutModalConfirmText, { color: colors.primaryForeground }]}
                  >
                    Done
                  </Text>
                </Pressable>
                ...
                >
                  <Text style={[styles.logoutModalCancelText, { color: colors.destructive }]}>
                    Sign Out
                  </Text>
                </Pressable>
              </View>
          ...
          <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                />
                <Text
                  style={[styles.logoutModalTitle, { marginBottom: 0, color: colors.foreground }]}
                >
        ...
        {isOffline ? (
          <View
            style={[
              ...
              { backgroundColor: colors.warning + '15', borderBottomColor: colors.warning + '44' },
            ]}
          >
            ...
            <View style={styles.searchBarButtons}>
              ...
              {hasResults || filters.keywords ? (
                <Pressable
                  ...
                  style={[
                    styles.secondaryBtn,
                    styles.searchBarClearBtn,
                    { borderColor: colors.border },
                  ]}
                >
                  <Text style={[styles.searchBarClearBtnText, { color: colors.mutedForeground }]}>
                    Clear
                  </Text>
                </Pressable>
              ) : null}
