...
the fail-closed `CI / required` aggregator. GitHub settings and live run status
are not inferred from this file.
...
| protected-map-timeout-contract | `CI / required` → `pnpm run test-standard-plus` | direct portable contract for protected-map timeout escalation, suite ownership, and descendant cleanup | PR, merge queue, main push, manual | tracked contract test |
...
| dead-code-policy-contract | `CI / required` → `pnpm run test-standard-plus` | requires every validation package to declare Knip coverage or a documented exclusion, including scripts entrypoints and generated helpers | PR, merge queue, main push, manual | tracked contract test |
...
The scheduled audit is maintenance-only; README synchronization is the only
write-capable workflow and accepts no pull-request event.
