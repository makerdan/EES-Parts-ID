...
| Required validation | `verified` | `main` requires strict status context `CI / required`; the tracked aggregator fails closed unless portable validation succeeds on pull-request and merge-group paths. This task changes repository workflow contracts only, not GitHub settings. |
