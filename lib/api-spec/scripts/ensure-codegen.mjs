...
import { checkGeneratedOutputs, formatGeneratedOutputFailures } from "./generated-output-check.mjs";
...
function main() {
  // Re-entrancy guard: the real `codegen` script runs `pnpm -w run typecheck:libs`
  // as its final step, and `typecheck:libs` now has a `pretypecheck:libs` hook that
  // invokes this guard. Without this early return, a guard-triggered codegen would
  // spawn `codegen`, whose nested `typecheck:libs` would re-enter this guard and
  // block forever waiting for the lock the outer guard already holds. `codegen`
  // sets CODEGEN_ENSURE_SKIP=1 for that nested run so it short-circuits here.
